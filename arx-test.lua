...
GameEndRemote.OnClientEvent:Connect(function()
    if hasSentWebhook == true then
     ...
     gameRunning = false
    resetUpgradeOrder() -- Reset for next game
    hasSentWebhook = true
...
if player.PlayerGui:FindFirstChild("GameEndedAnimationUI") then
   ...
   -- local rewardsText = buildRewardsText()
    ...
    notify("✅ Webhook", "Stage completed and sent to Discord.")

 -- Check for boss ticket return first (highest priority)
    if pendingBossTicketReturn then
        ...
        pendingBossTicketReturn = false -- Reset the flag
        task.delay(2, function()
            TeleportService:Teleport(72829404259339, player)
        end)
        return -- Skip all other auto actions
    end
...
if pendingChallengeReturn then
        ...
        pendingChallengeReturn = false -- Reset the flag
        task.delay(2, function()
            TeleportService:Teleport(72829404259339, player)
        end)
        return -- Skip all other auto actions
